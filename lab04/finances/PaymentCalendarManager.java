...
public class PaymentCalendarManager {

    public static void showDialog(JFrame parent, Connection conn) {
        createScheduledPaymentsTable(conn);
        ...
        Runnable loadCalendar = () -> loadCalendar(conn, calendarTable, monthSpinner, eventsTable, allEventsTable);
        Runnable loadAllEvents = () -> loadAllEvents(conn, allEventsTable);
        Runnable loadEventsForSelectedDay = () -> loadEventsForSelectedDay(conn, calendarTable, eventsTable, monthSpinner);
        ...
        createAllBtn.addActionListener(e -> createAllMissedPayments(conn, monthSpinner, dialog, loadCalendar, loadAllEvents, loadEventsForSelectedDay));
        addEventBtn.addActionListener(e -> showEventDialog(conn, dialog, null, loadCalendar, loadAllEvents, loadEventsForSelectedDay));
        editEventBtn.addActionListener(e -> {
            int row = allEventsTable.getSelectedRow();
            if (row == -1) {
                JOptionPane.showMessageDialog(dialog, "Выберите событие в правой таблице.");
                return;
            }
            int id = (int) allEventsTable.getValueAt(row, 0);
            showEventDialog(conn, dialog, id, loadCalendar, loadAllEvents, loadEventsForSelectedDay);
        });
        ...
        createTransactionBtn.addActionListener(e -> {
            ...
            if (row == -1) {
                JOptionPane.showMessageDialog(dialog, "Выберите событие.");
                return;
            }
            ...
            if (selectedDate == null) {
                JOptionPane.showMessageDialog(dialog, "Выберите день в календаре.");
                return;
            }
            createTransactionFromEvent(conn, eventId, selectedDate, dialog);
            loadEventsForSelectedDay.run();
        });
    ...
    private static void createScheduledPaymentsTable(Connection conn) {
    ...
    private static JSpinner createMonthSpinner() {
    ...
    private static YearMonth getSelectedYearMonth(JSpinner spinner) {
    ...
    private static void loadCalendar(Connection conn, JTable calendarTable, JSpinner monthSpinner,
                                     JTable eventsTable, JTable allEventsTable) {
        ...
        for (int row = 0; row < 6; row++) {
            ...
            for (int col = 0; col < 7; col++) {
                if (row == 0 && col + 1 < startWeekday) rowData.add(null);
                    ...
                    rowData.add(new CalendarDayCell(dayCounter, paymentsByDay.getOrDefault(dayCounter, Collections.emptyList())));
                    dayCounter++;
                }
            }
        ...
        for (int i = 0; i < calendarTable.getColumnCount(); i++)
            calendarTable.getColumnModel().getColumn(i).setPreferredWidth(100);
    }

    private static LocalDate getSelectedDateFromCalendar(JTable calendarTable, YearMonth yearMonth) {
    ...
    private static void loadEventsForSelectedDay(Connection conn, JTable calendarTable,
                                                 JTable eventsTable, JSpinner monthSpinner) {
        YearMonth yearMonth = getSelectedYearMonth(monthSpinner);
        ...
        String sql = "SELECT sp.id, sp.name, sp.amount, c.name as category, sp.note " +
                "FROM scheduled_payments sp JOIN categories c ON sp.category_id = c.id WHERE sp.day_of_month = ?";
        try (PreparedStatement pst = conn.prepareStatement(sql)) {
            pst.setInt(1, day);
            ResultSet rs = pst.executeQuery();
            ...
            while (rs.next()) {
                model.addRow(new Object[]{rs.getInt("id"), rs.getString("name"), rs.getDouble("amount"), rs.getString("category"), rs.getString("note")});
            }
    ...
    private static void loadAllEvents(Connection conn, JTable allEventsTable) {
        String sql = "SELECT sp.id, sp.name, sp.amount, c.name as category, sp.day_of_month, sp.note " +
                "FROM scheduled_payments sp JOIN categories c ON sp.category_id = c.id ORDER BY sp.day_of_month";
        try (Statement st = conn.createStatement(); ResultSet rs = st.executeQuery(sql)) {
            ...
            while (rs.next()) {
                model.addRow(new Object[]{rs.getInt("id"), rs.getString("name"), rs.getDouble("amount"), rs.getString("category"), rs.getInt("day_of_month"), rs.getString("note")});
            }
    ...
    private static void showEventDialog(Connection conn, JDialog parent, Integer eventId,
                                        Runnable... refreshCallbacks) {
        ...
        gbc.gridx = 1;
        JComboBox<String> categoryCombo = new JComboBox<>(getCategories(conn));
        dialog.add(categoryCombo, gbc);
        gbc.gridx = 0; gbc.gridy = 2;
        dialog.add(new JLabel("Сумма:"), gbc);
        gbc.gridx = 1;
        JTextField amountField = new JTextField(15);
        dialog.add(amountField, gbc);
        gbc.gridx = 0; gbc.gridy = 3;
        dialog.add(new JLabel("День месяца (1-31):"), gbc);
        gbc.gridx = 1;
        JSpinner daySpinner = new JSpinner(new SpinnerNumberModel(1, 1, 31, 1));
        dialog.add(daySpinner, gbc);
        gbc.gridx = 0; gbc.gridy = 4;
        dialog.add(new JLabel("Примечание:"), gbc);
        gbc.gridx = 1;
        JTextArea noteArea = new JTextArea(3, 15);
        dialog.add(new JScrollPane(noteArea), gbc);
    ...
    private static void deleteEvent(Connection conn, int id) {
        try (PreparedStatement pst = conn.prepareStatement("DELETE FROM scheduled_payments WHERE id=?")) {
            pst.setInt(1, id);
            pst.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    private static void createTransactionFromEvent(Connection conn, int eventId, LocalDate date, JDialog parent) {
        try (PreparedStatement pst = conn.prepareStatement("SELECT name, category_id, amount, note FROM scheduled_payments WHERE id=?")) {
            ...
            if (rs.next()) {
                ...
                if (isTransactionAlreadyCreated(conn, date, eventId)) {
                    JOptionPane.showMessageDialog(parent, "Транзакция на эту дату уже существует для данного события.");
                    return;
                }
    ...
    private static boolean isTransactionAlreadyCreated(Connection conn, LocalDate date, int eventId) throws SQLException {
        String sql = "SELECT COUNT(*) FROM transactions t " +
                "JOIN scheduled_payments sp ON t.category_id = sp.category_id " +
                "WHERE t.date = ? AND sp.id = ?";
        try (PreparedStatement pst = conn.prepareStatement(sql)) {
    ...
    private static void createAllMissedPayments(Connection conn, JSpinner monthSpinner, JDialog parent,
                                                Runnable... refreshCallbacks) {
        YearMonth yearMonth = getSelectedYearMonth(monthSpinner);
        int daysInMonth = yearMonth.lengthOfMonth();
        int createdCount = 0;
        ...
        try (PreparedStatement pst = conn.prepareStatement(sql)) {
            ...
            if (createdCount > 0) {
                JOptionPane.showMessageDialog(parent, "Создано транзакций: " + createdCount);
                for (Runnable r : refreshCallbacks) r.run();
            } else {
                JOptionPane.showMessageDialog(parent, "Все платежи за месяц уже созданы.");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    private static String[] getCategories(Connection conn) {
    ...
    private static String getCategoryNameById(Connection conn, int id) {
    ...
    private static class ScheduledPayment {
        ...
        ScheduledPayment(int id, String name, double amount, String category, int dayOfMonth, String note) {
            ...
            this.dayOfMonth = dayOfMonth; this.note = note;
        }
    }
    ...
    private static class CalendarDayCell {
        ...
        public String toString() {
            ...
            if (!payments.isEmpty()) {
                sb.append("<br><font size='2' color='gray'>");
                for (ScheduledPayment p : payments) sb.append(p.name).append(" ").append(p.amount).append("<br>");
                sb.append("</font>");
            }
            sb.append("</html>");
            return sb.toString();
    ...
    private static class CalendarCellRenderer extends DefaultTableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
            ...
            if (value instanceof CalendarDayCell) {
                ...
                if (!((CalendarDayCell) value).payments.isEmpty())
