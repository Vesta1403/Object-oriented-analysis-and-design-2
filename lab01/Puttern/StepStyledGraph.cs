...
namespace WinFormsApp2
{
    ...
    public class StepStyledGraph : IGraph
    {
        public void Draw(PictureBox pictureBox, int[] X, int[] Y)
        {
            ...
            using (Graphics g = Graphics.FromImage(bmp))
            {
                ...
                // Вычисление границ 
                GraphUtils.ComputeBounds(drawX, drawY, 0, 1, bmp.Width, bmp.Height,
                ...
                {
                    //  Оси 
                    g.DrawLine(axisPen, 0, yZero, bmp.Width, yZero);  // ось X 
                    g.DrawLine(axisPen, xZero, 0, xZero, bmp.Height); // ось Y 
                    ...
                    points.Add(new PointF(mapX(minX), mapY(0))); // начальная точка слева

                    for (int i = 0; i < drawX.Length; i++)
                    {
                        double leftY = (i == 0) ? 0 : drawY[i - 1];
                        points.Add(new PointF(mapX(drawX[i]), mapY(leftY)));   // подъём
                        points.Add(new PointF(mapX(drawX[i]), mapY(drawY[i]))); // горизонталь на уровне узла

                        if (i < drawX.Length - 1)
                            points.Add(new PointF(mapX(drawX[i + 1]), mapY(drawY[i]))); // горизонталь к следующему узлу
                        else
                            points.Add(new PointF(mapX(maxX), mapY(drawY[i]))); // горизонталь справа
                    }
